...
def generate_output_json(analysis_result, output_file="output.json"):
    """
...
def run_jacoco_analysis(path: str = None):
    ...
    if path:
        ...
        if not detected_items or (detected_items and choice_idx >= len(detected_items)):
            ...
            if choice == "1":
                # Archive file
                console.print()
                archive_path = Prompt.ask("Enter path to JaCoCo archive (zip or 7z)")
                ...
                # Directory
                console.print()
                report_dir = Prompt.ask("Enter path to JaCoCo report directory")
                report_dir = report_dir.strip().strip('"').strip("'")
    ...
    try:
        ...
        if save_choice.lower() == "y":
            output_file = Prompt.ask("Output filename", default="jacoco_analysis.json")
...
def print_help():
    ...
    console.print(
        "    [green]--file, -f[/green]             Use local input.json file"
    )
    console.print(
...
def main():
    ...
    if saved_mode:
        mode_display = (
            "SonarCloud API" if saved_mode == "api" else "local input.json file"
        )
        console.print(f"[dim]Using saved selection:[/dim] [bold]{mode_display}[/bold]")
        console.print()
    ...
    console.print("    [green][2][/green] Use local input.json file")
    console.print(
        "    [green][3][/green] Analyze JaCoCo coverage report "
